...
def test_thorpe_dissipation_rate_output(dataframe_regression):
    ...
    dataframe_regression.check(thorpe_eps_df, default_tolerance=dict(rtol=1e-05, atol=1e-08))
...
def test_finestructure_dissipation_rate_output(dataframe_regression):
    ...
    dataframe_regression.check(eps_IGW_strain_df, default_tolerance=dict(rtol=1e-05, atol=1e-08))
...
def test_idemix_dissipation_rate_output(dataframe_regression):
    ...
    dataframe_regression.check(eps_IGW_IDEMIX_df, default_tolerance=dict(rtol=1e-05, atol=1e-08))
